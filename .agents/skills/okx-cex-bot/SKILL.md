...
## Token Selection Workflow

When the user asks to **research market data and pick the best spot grid / spot DCA candidates**, use a deterministic screening loop instead of guessing:

1. **Build a liquid universe**
  - Start with `okx market filter --instType SPOT --quoteCcy USDT --sortBy volUsd24h --limit 20`.
  - Exclude stablecoins, wrapped assets, obvious low-vol / thin-book pairs, and newly listed names with unstable prints.

2. **Gather objective signals for each candidate**
  - `okx market ticker <instId>` → 24h range, last price, change%
  - `okx market orderbook <instId> --sz 20` → spread + depth check
  - `okx market candles <instId> --bar 4H --limit 50` → range structure
  - `okx market indicator ema <instId> --bar 4H --params 5,20`
  - `okx market indicator macd <instId> --bar 4H`
  - `okx market indicator rsi <instId> --bar 4H --params 14`
  - `okx market indicator bb <instId> --bar 4H`

3. **Score candidates with an objective rubric**
  - **Liquidity**: higher `volUsd24h`, tighter spread, deeper book
  - **Range / volatility**: enough movement to monetize, but not gap-driven chaos
  - **Trend suitability**: EMA / MACD alignment and sensible RSI regime
  - **Risk penalty**: extreme RSI, wide spreads, erratic candles, or unstable new listings

  Suggested weights:
  - **Spot grid**: 40% liquidity, 25% range, 20% mean reversion, 15% spread / risk
  - **Spot DCA**: 40% liquidity, 30% trend, 20% pullback quality, 10% risk

4. **Choose the bot type from regime**
  - **Spot grid**: best when the token is liquid, range-bound, and mean-reverting (good spread, repeated swings, no runaway trend).
  - **Spot DCA**: best when the token is liquid and in a constructive uptrend with a healthy pullback setup.
  - If the top candidate is clearly trending, prefer **spot DCA**; if it is choppy / oscillatory, prefer **spot grid**.

5. **Return a shortlist before any order**
  - Present the top 3 candidates with a one-line rationale and the chosen bot type.
  - Do **not** claim guaranteed profits — only objective setup quality.
  - If the user wants a live order, ask for final confirmation after the shortlist is shown.

6. **Launch only after confirmation**
  - Create the bot with the selected `instId` only after the user approves the final candidate and mode.
